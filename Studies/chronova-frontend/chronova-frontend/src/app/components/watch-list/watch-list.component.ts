...
@Component({
  ...
  template: `
    <div class="container mt-4">
      ...
      <!-- Watch Grid -->
      <div class="row">
        <div class="col-md-4 mb-4" *ngFor="let watch of watches; trackBy: trackById">
  ...
  addToCart(watch: Watch) {
    ...
    this.apiService.addToCart(userId, cartItem).subscribe({
      next: (cart) => {
        ...
        // In a real app, you would use a toast notification or a more subtle indicator
        alert(`${watch.brandName} ${watch.modelName} has been added to your cart.`);
      },
      error: (error) => {
        console.error('Error adding to cart:', error);
        // Provide more specific error feedback if possible
        alert('Could not add watch to cart. Please try again later.');
      }
    });
  }

  trackById(index: number, watch: Watch): number {
    return watch.id || 0;
  }
}